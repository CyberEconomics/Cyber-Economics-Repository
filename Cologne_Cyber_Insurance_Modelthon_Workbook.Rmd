...
# of the verification set (32k).
 ...
 expected_loss_bonds = sum(bond_loss)/(num_sim_runs*n_runs),
 variance_loss_bonds = var(bond_loss),
 #Target Sharpe Ratio solves for coupon payments = TRSR * VAR + EV(Return) + Risk Free Rate
 coupon_payment = target_sharpe_ratio * variance_loss_bonds + expected_loss_bonds + risk_free_rate
 )
 
 summary_statistics_bonds <- data.frame(matrix(ncol = num_rc, nrow = 8))
 summary_statistics_bonds[1:3,1:num_rc] = bond_statistics[,2:4]
...
scenario_3_results_ils = calculate_reinsurance_ils_outcomes(constrained_optimization_results[1,,,], verification_set_new, scenario_3_IW, reinsurance_matrix_1,bond_value_array,2, 0.02)
