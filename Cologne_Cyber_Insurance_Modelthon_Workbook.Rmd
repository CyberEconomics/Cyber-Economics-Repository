...
#Load Var 95.0 Results
load(paste0(wd,"/data/premium_array.RData"))

#load ILS Scenario Data
load(paste0(wd,"/data/var95.0_data.RData"))

#load ILS Scenario Data
#load(paste0(wd,"/data/var90.0_data.RData"))
...
scenario_5_IW = c(10e3,5e3,2.5e3,1.25e3,0.625e3)
...
calculate_insurance_outcomes <- function(weights,verification_set,initial_wealth_array, mode){
  ...
  rownames(result_matrix) <- c("Initial Wealth (IW) in Thousand", "Avg. VaR 99.5% (RP200) in Thousand", "Sum of VaR-Violations", "Avg. Profit in Thousand", "Model Premium", "Implied Expense Ratio", "Avg. Probability of Loss" )
  ...
  realized_result <- mean(returnvector) + realized_result
...
realized_result <- mean(reinsurance_return_vector) + realized_result
...
calculate_reinsurance_ils_hypothetical_bond_outcome <- function(weights, verification_set, initial_wealth_array, reinsurance_matrix,bond_value,target_sharpe_ratio, risk_free_rate, VaR_Position_ILS){
  ...
  #Get Number of runs to calculate Var 99.5. Algorithm: Divide n_runs by 200 and multiply this number by 199 (position of VAR) rounded up
  n_runs =  ncol(verification_set)
  VAR_position = ceiling(n_runs * VaR_Position_ILS)
  VaR <- returnvector[VAR_position] + VaR
  realized_result <- mean(returnvector) + realized_result
  #Premiums need to be scaled down by 1000. The model premium is calculated by 
  model_premium_without_expense_ratio = (colSums(active_weight %*% (premium_array))) + model_premium_without_expense_ratio
...
n_runs =  length(reinsurance_return_vector)
adjusted_position = VaR_Position_ILS
VAR_position = n_runs - ceiling(n_runs * adjusted_position)
VaR <- reinsurance_return_vector[VAR_position] + VaR
realized_result <- mean(reinsurance_return_vector)  + realized_result
probability_of_loss = (length(returnvector[reinsurance_return_vector < 0]) / n_runs) + probability_of_loss
 ...
 reinsurance_var_array = data.frame(matrix(1:(3*num_rc), ncol = 3, nrow = num_rc))
 colnames(reinsurance_var_array) <- c("Number_RC", "VAR99.5", "Effective_IW")
 for (i in 1:num_rc){
   reinsurance_var_array[i,1] = i
   reinsurance_var_array[i,2] = reinsurance_result_matrix[1,i]
   reinsurance_var_array[i,3] = sum(reinsurance_matrix[i,] * output_insurance[[1]][1,]) *(-1)
 ...
 #New January 2024: Calculate the share of IW for the reinsurer by multiplying the reinsurance matrix by the repsective IWs of the primary companies
...
#Lookup VaR and Bond Value
 ...
 #Calculate excess loss payout & residual (=PPP Scheme)
 bond_outcomes = bond_outcomes %>% group_by(Number_RC) %>% mutate(
   excess_var_loss = ifelse(VAR99.5 - as.numeric(Profit) > 0,VAR99.5 - as.numeric(Profit),0),
   bond_loss = ifelse(excess_var_loss > Cyber_Bond_Value, Cyber_Bond_Value,excess_var_loss),
   excess_uninsured_loss = excess_var_loss - bond_loss,
   excess_var_loss_IW = ifelse(Effective_IW - as.numeric(Profit) > 0,Effective_IW - as.numeric(Profit),0),
   bond_loss_IW = ifelse(excess_var_loss_IW > Cyber_Bond_Value, Cyber_Bond_Value,excess_var_loss_IW),
   excess_uninsured_loss_IW = ifelse(excess_var_loss_IW > Cyber_Bond_Value, Cyber_Bond_Value,excess_var_loss_IW)
...
# of the verification set (32k).
 ...
 absolute_expected_loss_bonds_IW = mean(bond_loss_IW),
 relative_expected_loss = mean(absolute_expected_loss_bonds/Cyber_Bond_Value),
  relative_expected_loss_IW = mean(absolute_expected_loss_bonds_IW/Cyber_Bond_Value),
 sd_loss_bonds = sd(bond_loss),
  sd_loss_bonds_IW = sd(bond_loss_IW),
 #Target Sharpe Ratio solved for coupon payments = TRSR * VAR + EV(Return) + Risk Free Rate
coupon_payment = target_sharpe_ratio * sd_loss_bonds + absolute_expected_loss_bonds + risk_free_rate,
coupon_payment_IW = target_sharpe_ratio * sd_loss_bonds_IW + absolute_expected_loss_bonds_IW + risk_free_rate,
Return_On_Equity_Investors = coupon_payment/Cyber_Bond_Value,
Return_On_Equity_Investors_IW = coupon_payment_IW/Cyber_Bond_Value,
sum_uninsured_losses =  mean(excess_uninsured_loss),
sum_uninsured_losses_IW = mean(excess_uninsured_loss_IW),
Attatchment_Probability_Bond = mean(ifelse(bond_loss > 0,1,0)),
Attatchment_Probability_Bond_IW = mean(ifelse(bond_loss_IW > 0,1,0)),
Probability_of_excess_loss = mean(ifelse(excess_uninsured_loss > 0,1,0)),
Probability_of_excess_loss_IW = mean(ifelse(sum_uninsured_losses_IW > 0,1,0))
 )
...
values = pivot_longer(values, cols = c("Initial Wealth (IW) in Thousand", "Avg. VaR 99.5% (RP200) in Thousand", "Sum of VaR-Violations", "Avg. Profit in Thousand", "Model Premium", "Implied Expense Ratio", "Avg. Probability of Loss" ))
...
### Graphing Functions
plot_22_reinsurance <- function(data) {
  
  x_values <- c("X1", "X2", "X3", "X4")
  
  y_values <- c("Avg. VaR 99.5% (RP200)", "Avg. Profit", 
                "Primary Model Premium Share", "Avg. Probability of Loss")
  
  df <- data.frame(x = factor(rep(x_values, each = length(y_values))),
                   y = unlist(data[y_values]))
...
reinsurance_matrix_1 = rbind(c(0,0,1,0,0),c(0,0,1,0,0),c(0,0,1,0,0),c(0,0,1,0,0))
...
bond_value_array = c(21,11,5.25,2.6) 
bond_value_array = c(2000,5000,8000,10000)

#ILS effectivness
scenario_3_results_ils_unconstrained = calculate_reinsurance_ils_outcomes(unconstrained_results[1,,,], verification_set, scenario_3_IW, reinsurance_matrix_1,bond_value_array,0.7, 0.02)
scenario_3_results_ils_constrained = calculate_reinsurance_ils_outcomes(constrained_results[1,,,], verification_set, scenario_3_IW, reinsurance_matrix_1,bond_value_array,0.7, 0.02)

#ILS effectivness
scenario_3_results_ils_unconstrained_curve = calculate_reinsurance_ils_outcomes(unconstrained_results[1,,,], verification_set, scenario_3_IW, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.7, 0.02)
scenario_3_results_ils_constrained_curve = calculate_reinsurance_ils_outcomes(constrained_results[1,,,], verification_set, scenario_3_IW, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.7, 0.02)

#ILS Outcomes with Reinsurance Outcomes
reinsurance_matrix_1 = rbind(rep(0.5,3),rep(0.5,3),rep(0.5,3),rep(0.5,3))

scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(unconstrained_results[1,,,], verification_set, IW_array, reinsurance_matrix_1,bond_value_array,0.7, 0.02,.995)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(constrained_results[1,,,], verification_set, IW_array, reinsurance_matrix_1,bond_value_array,0.7, 0.02,.995)

#ILS Outcomes with Reinsurance Outcomes
scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(unconstrained_results[1,,,], verification_set, IW_array, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.7, 0.02,.995)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(constrained_results[1,,,], verification_set, IW_array, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.7, 0.02,.995)
...
#Start Analysis Jan 2024, Value at riskis is at 95% insteadof 99.5%.
#unconstrained_results 
#constrained_results
#capital_curve_new 
#IW_array = c(1e4, 5e3, 2.5e3,1e3,5e2)
...
reinsurance_matrix_1 = rbind(rep(0.25,5),rep(0.25,5),rep(0.25,5),rep(0.25,5))
reinsurance_matrix_1 = rbind(c(1,1,1),c(1,1,1),c(1,1,1))
IW_array_var95 =  c(10e3,5e3,2.5e3)

test_outcome1_unconstrained = calculate_insurance_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, 1)

test_outcome1_constrained = calculate_insurance_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, 1)


test_outcome2_unconstrained = calculate_reinsurance_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1, 0)

test_outcome2_constrained = calculate_reinsurance_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1, 0)
...
bond_values_effiency_curve_ils = c(2500,2500,2500)
bond_value_array = c(5000,5000,5000)

#ILS effectivness
scenario_3_results_ils_unconstrained = calculate_reinsurance_ils_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_value_array,0.6, 0.04)
scenario_3_results_ils_constrained = calculate_reinsurance_ils_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_value_array,0.6, 0.04)

#ILS effectivness
scenario_3_results_ils_unconstrained_curve = calculate_reinsurance_ils_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04)
scenario_3_results_ils_constrained_curve = calculate_reinsurance_ils_outcomes(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04)

#ILS Outcomes with Reinsurance Outcomes
scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_value_array,0.6, 0.04,.95)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_value_array,0.6, 0.04,.95)

#ILS Outcomes with Reinsurance Outcomes
scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04,.95)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var95.0_data[1,,,], verification_set, IW_array_var95, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04,.95)
...
reinsurance_matrix_1 = rbind(rep(0.25,5),rep(0.25,5),rep(0.25,5),rep(0.25,5))
reinsurance_matrix_1 = rbind(c(1,1,1),c(1,1,1),c(1,1,1))
IW_array_var90 =  c(10e3,5e3,2.5e3)

test_outcome1_unconstrained = calculate_insurance_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, 1)

test_outcome1_constrained = calculate_insurance_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, 1)


test_outcome2_unconstrained = calculate_reinsurance_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1, 0)

test_outcome2_constrained = calculate_reinsurance_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1, 0)

test_1  = calculate_reinsurance_ils_hypothetical_bond_outcome(constrained_results[1,,,], verification_set, IW_array, reinsurance_matrix_1,bond_value_array,0.7, 0.02,.995)


test_1 = calculate_reinsurance_ils_hypothetical_bond_outcome_bulk(constrained_results[1,,,], verification_set, IW_array, list(reinsurance_matrix_1,reinsurance_matrix_2,reinsurance_matrix_3),list(bond_value_array,bond_value_array_2,bond_value_array_3),c(0.6,0.65,0.7), 0.02,.995)


calculate_reinsurance_ils_hypothetical_bond_outcome_bulk <- function(weights, verification_set, initial_wealth_array, reinsurance_matrix, bond_value,target_sharpe_ratio, risk_free_rate) {
  #First build the result table, by checking the lengths of the paramters.
  
  number_rows = length(bond_value)
  number_cols = length(target_sharpe_ratio)
  number_tables = length(reinsurance_matrix_1)
  
  result_array = array(number_rows,number_cols,number_tables)
 
#Outer Loop for full tables
  
  for (i in number_tables) {
    
#Inner Loop for cols
    
    for (j in number_cols) {
      
    }
     for (k in number_rows) {
       
  result_array[k,j,i] =   calculate_reinsurance_ils_hypothetical_bond_outcome(weights, verification_set, initial_wealth_array, reinsurance_matrix[number_tables], bond_value[number_rows],target_sharpe_ratio[number_cols], risk_free_rate)$`ILS Outcomes`[[1]]$coupon_payment[1]
...
bond_value_array = c(21,10.5,5.25,2.6) 
bond_values_effiency_curve_ils = c(500,1000,2500)
bond_value_array =c(2500,2500,2500)

#ILS effectivness
scenario_3_results_ils_unconstrained = calculate_reinsurance_ils_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_value_array,0.6, 0.04)
scenario_3_results_ils_constrained = calculate_reinsurance_ils_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_value_array,0.6, 0.04)

#ILS effectivness
scenario_3_results_ils_unconstrained_curve = calculate_reinsurance_ils_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04)
scenario_3_results_ils_constrained_curve = calculate_reinsurance_ils_outcomes(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04)

#ILS Outcomes with Reinsurance Outcomes
scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_value_array,0.6, 0.04,.90)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_value_array,0.6, 0.04,.90)

#ILS Outcomes with Reinsurance Outcomes
scenario_3_results_ils_RP200_unconstrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04,.90)
scenario_3_results_ils_RP200_constrained = calculate_reinsurance_ils_hypothetical_bond_outcome(var90.0_data[1,,,], verification_set, IW_array_var90, reinsurance_matrix_1,bond_values_effiency_curve_ils,0.6, 0.04,.90)
