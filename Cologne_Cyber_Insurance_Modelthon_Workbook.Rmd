...
#Write Reinsurance Outcomes to Data, fit to reinsurance
        ...
        if(i*k == 1){ 
       statistical_distributions_reinsurance =    as.data.frame(cbind(reinsurance_return_vector, rep(paste0((reinsurance_matrix_1[i,])), n_runs), rep(k, n_runs), c(1:n_runs), rep(i,n_runs)))
         colnames(statistical_distributions_reinsurance) <- c("Profit", "List_of_Weights", "Simulation_Number", "Damage_Year/Epoch", "Number_RC")   
    } else {
      new_rows = as.data.frame(cbind(reinsurance_return_vector, rep(paste0((reinsurance_matrix_1[i,])), n_runs), rep(k, n_runs), c(1:n_runs), rep(i,n_runs)))
         colnames(new_rows) <- c("Profit", "List_of_Weights", "Simulation_Number", "Damage_Year/Epoch", "Number_RC") 
      statistical_distributions_reinsurance = rbind(statistical_distributions_reinsurance, new_rows)
...
# of the verification set (32k).
 ...
 variance_loss_bonds = var(bond_loss),
 #Target Sharpe Ratio solved for coupon payments = TRSR * VAR + EV(Return) + Risk Free Rate
 coupon_payment = target_sharpe_ratio * variance_loss_bonds + absolute_expected_loss_bonds + risk_free_rate,
...
scenario_3_results_ils = calculate_reinsurance_ils_outcomes(constrained_optimization_results[1,,,], verification_set_new, scenario_3_IW, reinsurance_matrix_1,bond_value_array,1, 0.02)


#ILS Outcomes with Reinsurance Outcomes

scenario_3_results_ils_RP100 = calculate_reinsurance_ils_hypothetical_bond_outcome(constrained_optimization_results[1,,,], verification_set_new, scenario_3_IW, reinsurance_matrix_1,bond_value,1, 0.02,.99)
...
browser()
p = ggplot(data = values) + geom_bar(aes(y=as.numeric(`Initial Wealth (IW)`),x=(1)), stat="identity")

p = p + geom_bar(aes(y=as.numeric(get(names(values)[2]))*(-1),x=(1)), stat="identity",width=.5, position = "dodge") + facet_grid(. ~Number_RC) 
...
colors = c("red","blue","green","yellow","black")
for(simu_nb in seq(1,5)){
  df_to_plot_simu_1= df_to_plot[df_to_plot$Simulation_Number== simu_nb,]%>% filter(Number_RC == num_RC) %>% arrange(Profit)


  
p = p + theme_bw() + geom_line(data = df_to_plot_simu_1,aes(y=as.numeric(Profit),x=seq(1,length(Profit))),color = colors[simu_nb], label = paste0("simulation_nb",str(simu_nb)))+ggtitle(paste0("Profits by simulations for given RC"))+ xlab(" ") 
}
#ggsave(paste0("./IW_",IW,".png"),p)
print(p)
}
...
colors = c("red","blue","green","yellow","black")
for(simu_nb in seq(1,5)){
  df_to_plot_simu_1= df_to_plot[df_to_plot$Simulation_Number== simu_nb,]%>% filter(Initial_Wealth == IW) %>% arrange(as.numeric(Profit))
p = p + theme_bw() + geom_line(data = df_to_plot_simu_1,aes(y=as.numeric(Profit),x=seq(1,length(Profit))),color = colors[simu_nb], label = paste0("simulation_nb",str(simu_nb)))+ggtitle(paste0("Profits by simulations for initial wealth = ",IW))+ xlab(" ") 
}
#ggsave(paste0("./IW_",IW,".png"),p)
print(p)
}
...
p = p + theme_bw() + geom_bar(data = values,aes(y=as.numeric(`Initial Wealth (IW)`),x=(Number_RC),color = colors[Number_RC], stat='identity', label = paste0("Summary Statistics",str(simu_nb)))) + ggtitle(paste0("Profits by simulations for initial wealth = ",IW))+ xlab(" ") 
