...
scenario_4_IW = c(3.6e4)
scenario_4_CC_IW = c(3.6e4 + 0.135e4,3.6e4 + 0.135e4*2,3.6e4 + 0.135e4*3,3.6e4 + 0.135e4*4 )
...
#Similar to the capital curve, we need to explicitly declare the array, because if else, the last (4th) dimension of the original array gets wiped.  


#ils_pseudo_array = array(dim = 
#c(dim(ils_bond_scenario_1)[[2]],dim(ils_bond_scenario_1)[[3]],2))

ils_pseudo_array = array(dim = c(dim(SimulationresultArray)[[2]],dim(SimulationresultArray)[[3]],2))


#Get the results of the main optimization
ils_pseudo_array[,,1] = SimulationresultArray[1,,,1]
ils_pseudo_array[,,2] = SimulationresultArray[1,,,1]


#Bond Value, double matrix for input

ils_reinsurance_matrix = matrix(c(1,1), nrow = 1, ncol = 2)
bond_value = c(0.135e4)
...
dim_cc = dim(ils_bond_scenario_2)[[1]]
capital_curve_ils_pseudo_array = array(dim = c(dim(ils_bond_scenario_2)[[2]],dim(ils_bond_scenario_2)[[3]],2)) 

current_values_cc = matrix(nrow = 7, ncol = dim_cc)

for (i in 1:dim_cc){
  #Calculate Value and bind to new frame. First seed the pseudo array with the double rows and return
  capital_curve_ils_pseudo_array[,,1] = ils_bond_scenario_2[i,,,]
  capital_curve_ils_pseudo_array[,,2] = ils_bond_scenario_2[i,,,]
  current_values_cc = calculate_insurance_outcomes(capital_curve_ils_pseudo_array, verification_set, scenario_4_CC_IW[i])
capital_curve_ils_pseudo_array[,,i] = current_values_cc[[1]][4,1]
  
}
print(ils_capital_curve_overview)

#Capital Curve

profit_insurer_no_bond = ils_bond_results_1[[1]][[1]][4,1]
...
```{r}
ils_pseudo_array = array(dim = c(dim(ils_bond_scenario_1)[[2]],dim(ils_bond_scenario_1)[[3]],2))
ils_pseudo_array[,,1] = ils_bond_scenario_1[,,,1]
ils_pseudo_array[,,2] = ils_bond_scenario_1[,,,1]
...
ils_reinsurance_matrix = matrix(c(1,1), nrow = 1, ncol = 2)
bond_value = c(0.135e4)
ils_bond_results_1 = calculate_reinsurance_ils_hypothetical_bond_outcome(ils_pseudo_array, verification_set, scenario_4_IW, ils_reinsurance_matrix,bond_value,1, 0.02,.99)

#Capital Curve ILS

dim_cc = dim(SimulationresultArray)[[1]]
capital_curve_ils_pseudo_array = array(dim = c(dim(SimulationresultArray)[[2]],dim(SimulationresultArray)[[3]],2)) 

capital_curve_overview = matrix(nrow = 7, ncol = 1)
...
```{r}
#Start Graphics
plot_insurance_distributions(scenario_3_results_ils_RP100)
#Opposed to the Insurance Function, the Reinsurance Plot Funtion does not #subset after simulationsnumbers, as these are basically fixed to single
#insurance outcomes, which have a different arrangement 
plot_reinsurance_distributions(scenario_3_results_ils_RP100)
plot_insurance_summary_statistics(scenario_3_results_ils_RP100)
plot_ils_distributions(scenario_3_results_ils_RP100)
