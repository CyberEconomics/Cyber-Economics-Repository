...
#' @output Returns a list with tons of good stuff!
#'
calculate_reinsurance_ils_outcomes <- function(weights, verification_set, initial_wealth_array, reinsurance_matrix,bond_value_array,target_sharpe_ratio, risk_free_rate){
   #Code Equal to insurance_outcomes function at the end.
  num_IC =  dim(weights)[3]
  num_sim_runs =  dim(weights)[1]
  result_matrix <- data.frame(matrix(ncol = num_IC, nrow = 7))
...
probability_of_loss = (length(returnvector[reinsurance_return_vector < 0]) / n_runs) + probability_of_loss
 ...
 bond_value_array = data.frame(cbind(1:length(bond_value_array),bond_value_array))
 colnames(bond_value_array) <- c("Number_RC", "Cyber_Bond_Value")
  bond_value_array[,1] = sapply(bond_value_array[,1], as.factor)
 
#Lookup VaR and Bond Value
 bond_outcomes =  statistical_distributions_reinsurance %>% left_join(reinsurance_var_array, by = c("Number_RC" = "Number_RC"), copy = T) %>% left_join(bond_value_array, by = c("Number_RC" = "Number_RC"), copy = T)
...
# of the verification set (32k).
 ...
 Probability_of_bond_payout = count(bond_loss > 0)/(num_sim_runs*n_runs),
 Probability_of_excess_loss = count(sum_uninsured_losses > 0)/(num_sim_runs*n_runs),
 )
 #Create the summary Statistics for the ILS-Investors and Excess losses.
 summary_statistics_bonds <- data.frame(matrix(ncol = num_rc, nrow = 8))
 summary_statistics_bonds[1:4,1:num_rc] = bond_statistics[,1:4]
 ...
 final_output = list(output_insurance,output_reinsurance,bond_outcomes,bond_statistics,summary_statistics_bonds)
 names(final_output) = list("Insurance Outcomes", "Reinsurance Outcomes")
