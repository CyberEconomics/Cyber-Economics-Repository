...
#' @output Returns a list with tons of good stuff!
#'
calculate_reinsurance_ils_outcomes <- function(weights, verification_set, initial_wealth_array, reinsurance_matrix,bond_value_array,target_sharpe_ratio){
   #Code Equal to insurance_outcomes function at the end.
  num_IC =  dim(weights)[3]
  num_sim_rums =  dim(weights)[1]
  result_matrix <- data.frame(matrix(ncol = num_IC, nrow = 7))
...
#Lookup VaR and Bond Value
 ...
 #Filter down all outcomes to only the ones violating the respctive VAR-border and calculate excess loss payout & residual (=PPP Scheme)
 bond_outcomes = bond_outcomes %>% ungroup() %>% filter(as.numeric(Profit) < VAR99.5) %>% mutate(
   excess_var_loss = VAR99.5 - as.numeric(Profit),
   bond_loss = iflese(excess_var_loss > Cyber_Bond_Value, Cyber_Bond_Value,excess_var_loss)
 ...
 final_output = list(output_insurance,output_reinsurance,bond_outcomes)
 names(final_output) = list("Insurance Outcomes", "Reinsurance Outcomes")
...
scenario_3_results_ils = calculate_reinsurance_ils_outcomes(constrained_optimization_results[1,,,], verification_set_new, scenario_3_IW, reinsurance_matrix_1,bond_value_array,2)
